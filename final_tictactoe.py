...
# This function will begin to play in "normal" mode where it will begin to defend by blocking the enemy (you)
def computer_move_normal(playerTwo):
        # Row 0:
        if grid[0][0] == grid[0][1] and grid[0][0] != " " and grid[0][2] == " ": 
            grid[0][2] = playerTwo
        
        if grid[0][2] == grid[0][1] and grid[0][2] != " " and grid[0][0] == " ":
            grid[0][0] = playerTwo

        # Row 1:
        if grid[1][0] == grid[1][1] and grid[1][0] != " " and grid[1][2] == " ": 
            grid[1][2] = playerTwo
        
        if grid[1][2] == grid[1][1] and grid[1][2] != " " and grid[1][0] == " ": 
            grid[1][0] = playerTwo
        
        # Row 2:
        if grid[2][0] == grid[2][1] and grid[2][0] != " " and grid[2][2] == " ": 
            grid[2][2] = playerTwo
        
        # Column 0:
        if grid[0][0] == grid[1][0] and grid[0][0] != " " and grid[2][0] == " ": 
            grid[2][0] = playerTwo
        
        if grid[2][0] == grid[1][0] and grid[2][0] != " " and grid[0][0] == " ":
            grid[0][0] = playerTwo
        
        #Colum 1:
        if grid[0][1] == grid[1][1] and grid[0][1] != " " and grid[2][1] == " ": 
            grid[2][1] = playerTwo
        
        if grid[2][1] == grid[1][1] and grid[2][1] != " " and grid[0][1] == " ": 
            grid[0][1] = playerTwo

        # Colum 2:
        if grid[0][2] == grid[1][2] and grid[0][2] != " " and grid[2][2] == " ": 
            grid[2][2] = playerTwo
        
        if grid[2][2] == grid[1][2] and grid[2][2] != " " and grid[0][2] == " ": 
            grid[0][2] = playerTwo
        
        # Diagonal, top left to bottomr right
        if grid[0][0] == grid[1][1] and grid[0][0] != " " and grid[2][2] == " ": 
            grid[2][2] = playerTwo
        
        if grid[2][2] == grid[1][1] and grid[2][2] != " " and grid[0][0] == " ": # Reveresed - places on grid[0][0]
            grid[0][0] = playerTwo
        
        # Diagonal, to right, to bottom left
        if grid[0][2] == grid[1][1] and grid[0][2] != " " and grid[2][0] == " ": 
            grid[2][0] = playerTwo
        
        if grid[2][0] == grid[1][1] and grid[2][0] != " " and grid[0][2] == " ": # Reversed - places on grid[0][2]
            grid[0][2] = playerTwo
            return
...
while True:
    ...
    if choice == "Player":
        ...
        if cont_game == 'no' or cont_game =='No':
            print("Thanks for playing, take care! :)")
            break
        else:
            clear_grid()
    elif choice == "Computer":
